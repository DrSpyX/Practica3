...
int main()
{
    ...
    try{
        ...
        while(fin.good()){              //lee caracter a caracter hasta el fin del archivo
            char temp=fin.get();
            if(fin.good()){
                datos += temp;     //Asigna cada caracter leido a la cadena de caracteres
            }
        }
    ...
    if(metodo==1){
        string tempCad;
        int temp = contCerosUnos(cadenaBinaria.substr(0,seed), seed);              //contiene mayor cantidad de numeros, 0=iguales, 1+cantidad 0, 2+cantidad 1
        ...
        for(int i=1;i<=cantGrupos;i++){                             //itera todos los grupos llamando la funcion 'invertBits' para invertir los bits necesarios segun el caso de cada grupo
            temp2 = temp;
            temp = contCerosUnos(cadenaBinaria.substr(i*seed,seed), seed);
            if(temp2==0){
